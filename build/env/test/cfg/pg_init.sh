...
BEGIN
    ...
    if \$3 IS NULL THEN
        EXECUTE FORMAT('GRANT CONNECT ON DATABASE $1 TO "%I"', username);
        IF \$4 IS TRUE THEN
            EXECUTE FORMAT('GRANT CREATE ON DATABASE $1 TO "%I"', username);
        END IF;
    ELSE
...
SELECT frieze_user_create(username:='$1_ro', password:='$1_ro', acl_group:='read');
SELECT frieze_user_create(username:='$1_rw', password:='$1_rw', acl_group:='read');
SELECT frieze_user_create(username:='$1_rw', password:='$1_rw', acl_group:='write');
